                    ...
                    if 1:
                        ...
                        if app_state is ROOM_STATE:
                            ...
                            for cluster_states in clusters.get():
                                max_cluster_len = max(max_cluster_len, len(cluster_states))
                                if len(cluster_states) >= min_cluster_state:
                                    cluster = np.array([states[i] for i in cluster_states])
                                    w = cluster[:, 3]
                                    if sum(w) > 0:
                                        x_c, y_c, alpha_c = np.average(cluster[:, : 3], axis=0, weights=w)
                            ...
                            print('zero cluster =\t\t', sum([1 if sum([states[i][3] for i in state_indices]) == 0 else 0 for state_indices in clusters.get()]))
                        ...
                        ## SHARED ####
                        loop_time = time.time() - loop_start_time
                        cv2.putText(img, 'STATE    ' + app_state, (5, HEIGHT - 5),
                                0, 0.5, (125, 125, 125))
